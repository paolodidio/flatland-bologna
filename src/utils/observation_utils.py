...
def split_in_sublists_of_size(data, N):
    return np.array([data[x:x+N] for x in range(0, len(data), N)])

def concatenate_zip(data, distance, agent_data):
    split_data = split_in_sublists_of_size(data,N_FEATURE_DATA)
    split_distance = split_in_sublists_of_size(distance,N_FEATURE_DISTANCE)
    split_agent_data = split_in_sublists_of_size(agent_data,N_FEATURE_AGENT_DATA)
    obs = []
    for node_data, node_distance, node_agent_data in zip(split_data,split_distance,split_agent_data):
        node = np.concatenate((node_data, node_distance, node_agent_data))
        obs = np.concatenate((obs, node))
    return obs
...
def normalize_observation(observation, tree_depth: int, observation_radius=0):
    ...
    distance = norm_obs_clip(distance, normalize_to_range=True)
    agent_data = np.clip(agent_data, -1, 1)
    normalized_obs = concatenate_zip(data, distance, agent_data)
    return normalized_obs